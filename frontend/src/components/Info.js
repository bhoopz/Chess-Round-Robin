...
import { getSingleTournament } from "../services/TournamentService";
...
function Info() {
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      const res = await getSingleTournament(tournamentName);
      setData(res.data);
                              ...
                              <tbody>
                                <tr>
                                  <td>
                                    ...
                                    <span>
                                      {pair.score[1]} {pair.pair[1].name}{" "}
                                      {`(${pair.pair[1][elo]})`}
                                      {item.roundOver
                                        ? pair.ratingPoints[1] < 0
                                          ? `(${pair.ratingPoints[1]})`
                                          : `(+${pair.ratingPoints[1]})`
                                        : null}
                                    </span>
