...
const createTournament = async (req, res) => {
  ...
  let reversedPlayersPairs = arraySwitch(
    JSON.parse(JSON.stringify(playersPairs))
  );
  ...
  if (foundTournament.length != 0) {
    return res.status(400).send({ message: "Tournament name already exists" });
  } else {
    ...
    await tournament.save().then(
      (tournament) => {
        res.status(200).send(tournament.name);
      },
      (err) => {
